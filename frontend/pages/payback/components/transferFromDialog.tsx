...
export default function TransferFromDialog() {
    ...
    const handleTransferFrom = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const gasEstimate: bigint = await contract.methods.transferFrom(req_data.from, req_data.to, req_data.amount).estimateGas({ from: account });
            ...
            const result = await contract.methods.transferFrom(req_data.from, req_data.to, req_data.amount).send({
                from: account,
